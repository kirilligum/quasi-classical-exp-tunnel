...
vector<vector<double>> electronic_population(vector<vector<vector<double>>> trajs, double n1, double n2, double n1_bin_end, double n2_bin_end, double n1_shift, double n2_shift){
  vvvd n1ts;
  transform(trajs.begin(),trajs.end(),back_inserter(n1ts),[=](vvd traj){
        vvd n1t;
        transform(traj.begin(),traj.end(),back_inserter(n1t), [=](vd point) {
          return vector<double>{point.front(), box(get_n{n1_shift}(point,1), n1, n1_bin_end)};
          });
        return n1t;
      });
  ...
  transform(trajs.begin(),trajs.end(),back_inserter(n2ts),[=](vvd traj){
        ...
        transform(traj.begin(),traj.end(),back_inserter(n2t), [=](vd point) {
          return vector<double>{point.front(), box(get_n{n2_shift}(point,2), n1, n2_bin_end)};
          });
        return n2t;
      });
